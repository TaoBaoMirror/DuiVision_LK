		...
		if(m_rc.PtInRect(point))
		{
			...
			if(rc.PtInRect(point))	// 点击的是编辑框右部的小按钮图片
			{
				if(m_bIsSmallButton)	// 小图片作为按钮才进行处理
				{
					...
					//隐藏软键盘
					CString strTrayDbClickMsg = DuiSystem::Instance()->GetConfig(_T("keybord"));
					if(strTrayDbClickMsg == _T("1")&&!m_bIsDisable&&!m_bReadOnly)
					{
						CDuiKeyBord* pkDlg =(CDuiKeyBord*) DuiSystem::Instance()->GetDuiDialog(_T("dlg_keybord"));	
						if (pkDlg!=NULL)//隐藏软键盘
							pkDlg->ShowWindow(SW_HIDE);		
					}
				}
			}
			else
			{
				...
				if(strTrayDbClickMsg == _T("1")&&!m_bIsDisable&&!m_bReadOnly)
				{
					CDuiKeyBord* pDlg =(CDuiKeyBord*) DuiSystem::Instance()->GetDuiDialog(_T("dlg_keybord"));	
					if (pDlg!=NULL)// 弹出软键盘消息
					{
						CDlgBase *pParent=DuiSystem::Instance()->GetDuiDialog(_T("dlg_main"));
						CRect rc=GetRect();
						if (this->GetParentDialog(TRUE)!=pParent)
						{
							ClientToScreen(this->GetParentDialog(TRUE)->GetSafeHwnd(),(LPPOINT)&rc.TopLeft());
							ClientToScreen(this->GetParentDialog(TRUE)->GetSafeHwnd(),(LPPOINT)&rc.BottomRight());
						}					
						CRect dlgrc=pDlg->GetRect();
						CRect rcwnd=pParent->GetRect();
						ClientToScreen(pParent->GetSafeHwnd(),(LPPOINT)&rcwnd.TopLeft());
						ClientToScreen(pParent->GetSafeHwnd(),(LPPOINT)&rcwnd.BottomRight());

						dlgrc.left=rcwnd.left;
						dlgrc.right=rcwnd.right;
						if(rc.bottom>rcwnd.bottom-400)
						{
							int h=dlgrc.Height();
							dlgrc.bottom=rc.top-10;
							dlgrc.top=dlgrc.bottom-h;	
						}
						else
						{
							int h=dlgrc.Height();
							dlgrc.bottom=rcwnd.bottom;
							dlgrc.top=dlgrc.bottom-h;	
						}	
//						ClientToScreen(pParent->GetSafeHwnd(),(LPPOINT)&dlgrc.TopLeft());
//						ClientToScreen(pParent->GetSafeHwnd(),(LPPOINT)&dlgrc.BottomRight());
						SetWindowPos(pDlg->GetSafeHwnd(),HWND_TOPMOST,dlgrc.left,dlgrc.top,dlgrc.Width(),dlgrc.Height(),SWP_SHOWWINDOW);
						pDlg->ShowWindow(SW_SHOW);
						pDlg->SetInputControl(this);
					}			
				}
			}		
		}
