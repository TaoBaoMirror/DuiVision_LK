...
CControlBase* DuiSystem::CreateControlByName(LPCTSTR lpszName, HWND hWnd, CDuiObject* pParentObject)
{
	...
	CREATE_DUICONTROL_BY_CLASS_NAME(CDuiGroupBox);
...
CDlgBase* DuiSystem::CreateDuiDialog(LPCTSTR lpszXmlTemplate, CDuiObject* pParentObject, CString strName, BOOL bModule, UINT nIDTemplate, BOOL bAdd)
{
	...
	CDlgBase* pDlg =NULL;
	if(0==lstrcmpi(lpszXmlTemplate,_T("dlg_keybord")))
		pDlg=new CDuiKeyBord(nIDTemplate, pParentDlg);
	else
		pDlg=new CDlgBase(nIDTemplate, pParentDlg);
...
void DuiSystem::CreateKeyBord(LPCTSTR lpszXmlTemplate, CString strName /*= _T("")*/)
{
	if(m_pDlgKeyBord != NULL)
	{
		RemoveDuiDialog(m_pDlgKeyBord);
		m_pDlgKeyBord = NULL;
	}
	m_pDlgKeyBord = (CDuiKeyBord *)CreateDuiDialog(lpszXmlTemplate, NULL, strName, FALSE);
	if(m_pDlgKeyBord != NULL)
	{
		m_pDlgKeyBord->ShowWindow(SW_HIDE);
	}
}
