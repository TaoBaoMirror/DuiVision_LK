...
bool CDuiActiveX::DoCreateControl()
{
    ...
    if( FAILED(Hr) )
	{
		...
		if( FAILED(Hr) )
		{
			DuiSystem::LogEvent(LOG_LEVEL_ERROR, _T("CoCreateInstance  failed")/*, m_clsid*/);
		}
		// 控件激活
		else
...
//	m_clsid=CLSID_WebBrowser;
	//::CLSIDFromString(L"{D303AC37-74DB-45b9-8C22-AD7C3FBA68EF}", &m_clsid);	// IE浏览器控件ID
...
void CDuiFlashCtrl::OnAxActivate(IUnknown *pUnknwn)
{
	...
	if(m_bTransparent)
	{
		...
		flash_->put_WMode(bstr_t(_T("transparent")));
 		flash_->ScaleMode=2;
		flash_->put_AllowFullScreen(bstr_t(_T("true")));
